#!/usr/bin/env python3
"""Parse an external reference scheduler's text output, compute lex tuple, compare against our scheduler.

This script accepts a text-format schedule from any external FRC
scheduler that writes one match per line in the canonical text
format. The user is responsible for running the external scheduler
themselves; this script only consumes its text output for analysis.

External schedule text format (per match line):
    match_num  t1 s1  t2 s2  t3 s3  t4 s4  t5 s5  t6 s6
where t1-t3 are the red alliance, t4-t6 are the blue alliance, and
sN is the surrogate flag for team tN (0 or 1). Red is listed first
in the standard text dump used by established FRC schedulers.
"""
...
def parse_reference_schedule(path: str) -> list[Match]:
    """Parse a reference scheduler's text dump into a list of Match objects."""
    matches = []
    with open(path) as f:
        for line in f:
            line = line.strip()
...
def main():
    ref_path = sys.argv[1] if len(sys.argv) > 1 else '/mnt/user-data/uploads/state_qual_schedule.txt'

    ref_matches = parse_reference_schedule(ref_path)
    teams = set()
    for m in ref_matches:
        teams.update(m.red); teams.update(m.blue)
    n_teams = len(teams)
    print(f"Parsed {len(ref_matches)} matches; {n_teams} unique teams")
    print(f"Total team appearances: {sum(len(m.red) + len(m.blue) for m in ref_matches)}")
    print(f"Implied MPT: {sum(len(m.red) + len(m.blue) for m in ref_matches) / n_teams:.2f}")
    print()

    ref_tuple, ref_state = per_criterion_breakdown(ref_matches, n_teams, "Reference schedule (user-supplied)")
    ...
    print(f"  Reference schedule: {ref_tuple}")
