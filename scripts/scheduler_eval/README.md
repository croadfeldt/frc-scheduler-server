...
that does — possibly by simply importing reference scheduler output, possibly
...
### Optional: occasional manual the reference scheduler evaluation
...
```bash
# Install the reference scheduler manually, under your own evaluation license
# with the upstream tool's authors. Three ways for the harness to find it:
...
| matchmaker             | ✓ working, **evaluation-only** | Wraps the upstream tool's authors's externally-installed external reference binary. **Do not use in CI or automated pipelines** — see `NOTICE.md`. Appropriate for occasional manual evaluation runs to recalibrate the absolute quality ceiling. |
...
available played FRC schedules (which were generated by the reference scheduler via
...
adapters. The external reference scheduler handles surrogate placement itself (default
...
- The reviewer's reference-scheduler baseline (800 trials, 36-team field)
...
The reference scheduler, our scheduler, and CP-SAT are all stochastic — different
...
Comparing "best of 1000 reference-scheduler runs" to "best of 5 our-scheduler
...
foundation than the heuristic search both ours and the reference scheduler use."
