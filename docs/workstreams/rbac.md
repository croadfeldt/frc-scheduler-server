...
- `workstreams/schedule-lifecycle.md` — lifecycle, lock, freeze model that this RBAC layer gates
...
single `is_admin` flag proposed in `workstreams/schedule-lifecycle.md` Phase E.
...
- Replace the `is_admin` flag references in
  `workstreams/schedule-lifecycle.md` Phase E with role grants
...
`workstreams/schedule-lifecycle.md` — no retention policy, kept indefinitely.
