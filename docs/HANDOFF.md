...
bug.** `_lex_compare` (app/scheduler.py:1785) walks tuple elements
in order; `_sa_optimize` accept/reject (line 1995-2049) correctly
treats `diff_idx == 0` (cooldown) as always-reject for worsening
trades. Empirically verified on 12×6×2: across 5000 probed valid
2-swaps, zero cooldown-worsening trades were accepted. 37
acceptances were cooldown-improving, 10 were cooldown-equal with
lower-slot improvement.

The actual failure mode is **move-set-limited.** Instrumented SA
on 12×6×2 logged: drove cooldown from 11→7→3→2 in the first 25K
iterations, then **stuck at 2 for the remaining 475K iterations.**
The 2-swap neighborhood has no move that reduces cooldown below 2
from this state. Same pattern as Phase 5's station post-pass
diagnostic.

**Q4 architectural finding now empirically airtight:** SA-on-
greedy-construction structurally cannot satisfy FRC §10.5.2's
paramount criterion on tight fixtures — not because of a bug, but
because of fundamental design (greedy paints corners, 2-swap
can't escape). CP-SAT finds cooldown=0 trivially on the same
fixtures. No code changes from the audit. Production fixtures
(≥36 teams) reach cd=0 trivially. Captured in
`scheduler/phase1-f1a-sa-audit.md`. F1-c (CP-SAT-as-construction
prototype) is now strongly motivated and is the natural next
code-and-measure task — the architectural answer is empirically
strong enough to commit implementation effort.
...
| **F1-a SA paramount-cooldown audit: no bug, move-set-limited** (this session) | ✓ | Audited the SA's accept/reject for ADR 002 paramount-cooldown compliance. **No bug.** `_lex_compare` (app/scheduler.py:1785) walks tuple elements in order; `_sa_optimize` accept loop (line 1995-2049) correctly treats `diff_idx == 0` (cooldown) as always-reject for worsening trades. Empirically verified on 12×6×2: across 5000 probed valid 2-swaps, zero cooldown-worsening accepts — 37 acceptances were cooldown-improving, 10 were cooldown-equal with lower-slot improvement. The actual failure mode is **move-set-limited**: instrumented 500K-iter SA on 12×6×2 logged cooldown trajectory 11→7→3→2 in the first 25K iterations, then **stuck at 2 for the remaining 475K**. The 2-swap neighborhood has no move that reduces cooldown below 2 from typical greedy starts. Same pattern as Phase 5 station post-pass diagnostic. **Q4 architectural finding now empirically airtight**: SA-on-greedy structurally cannot satisfy FRC §10.5.2's paramount criterion on tight fixtures because of fundamental design (greedy paints corners, 2-swap can't escape) — not because of a bug. CP-SAT finds cooldown=0 trivially. Production fixtures (≥36 teams) reach cd=0 trivially; the pattern is small-fixture-only. **No code changes from this audit.** Captured in `scheduler/phase1-f1a-sa-audit.md`. F1-c (CP-SAT-as-construction prototype) is the natural next code-and-measure task; architectural answer is now strong enough to commit implementation effort. 13 test suites pass. |
