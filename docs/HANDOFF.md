...
| **Eval composite gap investigation + adapter fixes** (this session) | ◐ | First valid head-to-head eval (post-Bug-A) showed our scheduler losing 3W-0L to MatchMaker with mean composite 46.97 vs 27.00 (20-point gap). Investigation broke the gap down to three drivers: (1) **min_match_gap**: ours stuck at 3 (poor) vs MM's 5/6/7 (near-optimal). Adapter passed hardcoded `ideal_gap=3` while MM auto-computes max feasible gap from fixture geometry. (2) **max_color_imbalance**: ours at 3-4 (poor) vs MM at 3-4 (poor; tied) but our framework hits 1 with CP-SAT post-pass; adapter wasn't enabling it. (3) **max_station_spread**: ours at 3-5 (poor) vs MM at 1 (near-optimal). Two parts: CP-SAT post-pass would fix the framework's 6-position metric, but the harness measures a **different** 3-position metric (R1/B1 combined) — separate algorithmic gap. **Fixes in `scripts/scheduler_eval/adapters/frc_scheduler_server.py`**: (a) `ideal_gap` becomes fixture-aware via new `_resolve_ideal_gap()` method. Default heuristic `cooldown_max - 4` exactly matches MM's empirical choice (51×9→5, 55×9→6, 61×9→7). Explicit override via `ideal_gap=` kwarg preserved. (b) CP-SAT post-pass enabled by default via new `cpsat_post_pass_budget_s` kwarg defaulting to `DEFAULT_CPSAT_BUDGET_S=120.0`. (c) `adapter_diagnostics` enriched: records `ideal_gap`, `ideal_gap_source` ("fixture_floors.cooldown_max" or "override=N"), and `cpsat_post_pass_budget_s`. **Local validation** (500K SA + 120s CP-SAT on dev container): 51×9 min_gap 3→5 + station 4→1 + color 3→1, 55×9 min_gap 3→6 + station 3→1 + color 3→1, 61×9 min_gap 3→7 + station 5→1 + color 3→1. **Two residual issues documented** (Best Possible Schedule workstream candidates): Q1: opponent diversity regression — `repeat_opponents` went 16-44 → 71-92 as a side-effect of higher gap (constrained SA trades opponent variety for gap satisfaction). Q2: harness measures `max_station_spread` on 3 positions (R1/B1 combined); framework measures 6 positions (R1/R2/R3/B1/B2/B3 separate). Our CP-SAT post-pass optimizes the 6-position metric. Closing the harness metric to 1 requires either a 3-position post-pass or harness threshold tweak (after reading Sykes paper on station-balancing algorithm). **Doc**: new `docs/scheduler/eval-composite-gap.md` with full per-metric analysis. **Deploy notes**: change is in `scripts/scheduler_eval/` only (eval harness). No `--build`. No DB migration. **Status**: ◐ partial — adapter fixes ship cleanly, but full closure of the gap depends on Q1+Q2 algorithmic work. |
