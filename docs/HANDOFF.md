...
Then ran **F1-c** (CP-SAT-as-construction prototype). Built and
measured 6 fixtures × 2 arms at D5 cooldown=2. **Surprising
result**: identical lex tuples on every fixture where CP-SAT
terminated (12×6 par_quad=192 both arms; 16×6=106; 24×6=144;
36×7=252). CP-SAT killed by resource limits on 42×11 and 48×9.
**Production bug discovered**: `_sa_optimize` called
`_build_match_state(work)` without passing `ideal_gap`, so SA
always ran with cooldown threshold=3 regardless of caller intent.
Affected every production schedule since D5 set default to 2.
Fix shipped: `_sa_optimize` accepts `ideal_gap` kwarg (default 3
for back-compat); both production callers pass it. F1-a's "SA
can't reach cd=0 on 12×6" was an artifact of this bug.
**Q4 architectural answer**: keep greedy-then-SA. CP-SAT remains
research-only. ADR 003 stands.

Then started the **Schedule Quality Framework v1.0** workstream
(empirical validation framework for scheduling methodologies).
Phase A complete: theoretical floors module `app/quality_floors.py`
with `fixture_floors(n, mpt, tpa, cooldown)` returning per-metric
lower bounds. Each floor carries `confidence` (proven_optimal,
proven_lower_bound, or best_known) and `proof_note`. Covered:
cooldown_violations, surrogate_count, color_per_team,
station_per_team_spread, partner_pair_count, par_quad,
opponent_pair_count, opp_quad. Hand-validated against F1-c data:
24×6 and 36×7 hit the proven lower bound exactly; 12×6 cooldown=2
shows expected gap (floor=84, achieved=192) due to cooldown=2
forcing two non-interacting team groups. Re-exported through
...
Then shipped **Phase C** (per-criterion 0-100 scoring with
organizer-tunable weights + composite). Built
`app/quality_scoring.py` with per-metric curves: **binary**
for cooldown_violations (paramount-valid/invalid), **quadratic
decay** for par_quad/opp_quad (score = 100×(1-(distance/floor)²),
hits 0 at 2× floor), **linear-bounded** for rb/station/surrogate
(each +1 costs 25 points). `DEFAULT_QUALITY_WEIGHTS` is FRC
§10.5.2 priority-derived: cooldown/partner/opponent at 1.0,
surrogate/color/station at 0.5. **Paramount gate**: invalid
schedules get composite=0 regardless of other criteria.
**Weight clamping**: each weight in [0, 5]; cooldown auto-
clamped ≥ 1 (paramount can't be tuned away). **Best-known
floor scoring**: when a canonical exists for the shape, scoring
uses the canonical's achieved values as the floor — meaningful
for fixtures with structural gaps (12×6 cooldown=2's count-
floor is 84 but best-achievable is 192).

`build_quality_report()` extended to embed full scoring data
inline. Schema migration adds `quality_weights` JSONB column to
`abstract_schedules`. API: `POST /api/schedules` and
`POST /api/generate-abstract` accept `quality_weights`; **NEW**
`POST /api/abstract-schedules/{id}/rescore` re-scores existing
schedule under different weights without regenerating.

UI Quality card extended:
  - **Composite score badge** at top — color-graded
    Excellent/Good/Acceptable/Poor/Paramount-invalid with
    "Weights" button to open the editor
  - **Quality weights editor panel** — 6 per-criterion sliders
    (range 0-5, step 0.1), "Reset to defaults" + "Re-score"
    buttons
  - **Score column** added to the floor comparison table —
    color-coded per-criterion 0-100 score

All 5 canonicals rebuilt with Phase C scoring embedded.
`tests/test_quality_scoring.py` with 60+ assertions covers all
curves, weight normalization, composite math, paramount gate,
best-known override, integration with build_quality_report.

**Real finding surfaced**: rb_per_team and station_per_team_spread
post-passes at default 5000 iterations aren't reaching their
proven_optimal floors on larger fixtures (24×8 has rb=4 where
floor=0). The Phase C composite catches this — it'll be a
Phase D standing-eval finding when that workstream lands. New
`docs/scheduler/quality-scoring.md`. **Phase C complete.** 16
test suites pass.

Then shipped **Phase D** (standing eval suite — the runnable
gate). Built `scripts/scheduler_eval/standards.py` (runs
production scheduler on proving inventory, N seeds per fixture,
aggregates median composite + per-criterion, compares to bars,
writes JSON + Markdown reports). `scripts/scheduler_eval/
standards_config.py` defines bars — hard requirements
(paramount-valid, cooldown=100) apply universally; soft
thresholds (composite ≥ 80, per-criterion ≥ 50) overridable
per-fixture. CLI supports `--fixtures` subset, `--seeds` /
`--sa-iterations` configurable, `--strict` (CI mode: soft
becomes hard), `--smoke` (single-fixture sanity), `--no-report`
(skip file writes).

`tests/test_standards_smoke.py` with 50+ assertions covers
framework wiring (config, get_bars, run_one_seed,
evaluate_fixture, run_standards_eval, unknown-fixture filter).
The full standards.py is the operational tool; this test is
the CI sanity-check that it works end-to-end.

**Validated**: 12×6 cd=2 at 500K SA iter × 3 seeds reliably
passes (composite=100); 36×7 at 200K × 2 seeds passes
(composite=100). Smoke test (5K iter) fails by design — too
few iterations to converge. Catches the Phase-C-surfaced
rb/station post-pass weakness on larger fixtures as warnings.
New `docs/scheduler/quality-standards.md` documents bars,
how to run, how to interpret, how to update.

**Schedule Quality Framework v1.0 is fully shipped**:
  - Phase A (theoretical floors) → `app/quality_floors.py`
  - Phase B (canonical library + unified endpoint) →
    `app/canonical_library.py`, `POST /api/schedules`
  - Phase C (per-criterion + composite scoring with
    organizer-tunable weights) → `app/quality_scoring.py`,
    UI editor
  - Phase D (standing eval suite) →
    `scripts/scheduler_eval/standards.py`

The framework is operationally useful: any future scheduling
methodology runs through Phase D and gets compared on the
same terms. **Phase D complete.** 17 test suites pass.

**v1.1+ roadmap** (open for next session pick): higher-budget
canonical re-curation on Stark, CP-SAT producer mode for
proven_optimal upgrades, expanded ~27-shape FRC inventory,
...
| **Quality card consolidation + import flow popup** (this session) | ✓ | User-driven cleanup: align the Quality card's two parallel displays into a single actionable view, and add a popup at PDF/Excel import time so the user can pick between "new schedule" and "replace current schedule". **Quality card — single API**: extended `GET /api/abstract-schedules/{id}` to embed a `diagnostic` block (partner/opponent histograms, worst-pairs lists, slot table, pairs-never-met) inline, replacing the second `/diversity-report` round-trip. `loadDiversityReport()` is now single-fetch; the legacy endpoint stays for back-compat. **Quality card — single display**: replaced the four legacy headline tiles (avg partner repeats, avg opponent repeats, pairs never met, station balance) with **six framework tiles**, one per FRC §10.5.2 criterion (Cooldown, Partner, Opponent, Surrogate, Color, Station). Each tile shows: criterion name + FRC priority, 0-100 score (color-graded: green ≥90, amber 70-89, orange 50-69, red <50), raw value + floor + distance-from-floor, confidence label. Operational tiles (B2B, Surrogates count) preserved for editor click-to-filter behavior — IDs intact. **Floor comparison**: the old standalone floor-comparison table is removed; that information now lives in each criterion tile's detail line ("value: X · floor: Y (+Z) · confidence"). **Diagnostic detail**: histograms, worst-pairs, pairs-never-met moved into a "Diagnostic detail" expandable below the tiles (default collapsed) — preserves the why-is-the-score-what-it-is content without competing for the headline. **Card default-expanded**: confirmed `_scheduleQualityCollapsed` defaults to falsy → card opens expanded on every render. **Import popup**: added an import-mode picker to the PDF/Excel import preview modal with two radio buttons: "Create new schedule in this event" (default) and "Replace current schedule" (auto-disabled when no schedule is currently loaded, with a hint explaining why). Backend: extended `PdfImportCommitRequest` with `import_mode` and `target_schedule_id` fields; `POST /api/schedules/import-pdf/commit` validates the mode and dispatches. Replace path checks `_was_ever_official` (409 if so), checks lock state (423 if locked by other), calls `_snapshot_schedule_history(action='patch')` for reversibility, swaps the AssignedSchedule's `abstract_schedule_id` / `slot_map` / `day_config` / `practice_matches`, deletes & recreates MatchRow records. Frontend handles 409/423 with user-readable messages; success status differentiates "imported" vs "replaced". `refreshPdfImportModeUI()` keeps the radio enable/disable in sync with `_currentAssignedScheduleId`. **Tests**: new `tests/test_pdf_import_mode.py` with structural verification (model shape, endpoint validation, replace-path safeguards). 19 test suites pass. |
| **Exact CP-SAT post-passes + canonical library polish** (this session) | ✓ | Replace SA-based R/B + station post-passes with exact CP-SAT solvers for the canonical library curation path. The SA versions plateau on a meaningful fraction of seeds at modest iteration budgets (Phase D warned about this); CP-SAT solves both subproblems to provable optimality at modest budgets (60s total reaches composite=100 on every standards-inventory fixture, including 60×12). **Module**: new `app/post_passes/cpsat_post_passes.py` with `rb_balance_cpsat()` and `station_balance_cpsat()`. **R/B model**: M binary flip variables (one per match), lex objective `W·max + sum` of per-team \|red - blue\|. CP-SAT solves M=120 in 1-30s with parallel search. **Station model**: 2M alliances × 6 permutations of S_3 (one-hot encoded), per-team spread = max - min across 6 station positions, same lex objective. Provably preserves partner pairs, opponent pairs, cooldown, B2B, surrogate counts. **Budget cap**: `MAX_BUDGET_SECONDS = 3600` (1 hour). Solver respects user-given budget; on UNKNOWN (timeout without feasible solution), returns input unchanged — never makes things worse. **Empirical sweep**: new `scripts/cpsat_budget_sweep.py` runs the full 5-fixture × N-budget × M-seed matrix; results show CP-SAT never loses, beats SA by 2.8-5.6 composite points on ~25% of runs at 100K SA upstream. **Canonical producer integration**: `scripts/scheduler_eval/build_canonical.py` gains `--cpsat-post-pass-budget` and `--skip-cpsat-post-pass` flags. After SA-best-of-N selects the winner, CP-SAT polishes R/B (10% of budget, capped at 60s) and station (90% of budget). Auditability: methodology JSON records solver status, wall time, before/after max imbalances. **Canonical library rebuild**: all 5 canonical JSON files rebuilt with CP-SAT polish (200K SA iter × 3 seeds + 60-300s CP-SAT budget). Every canonical now scores composite=100 under the framework. The 36×7 and 60×12 canonicals show real improvement: 36×7 R/B max 3→1, station max 3→1; 60×12 R/B max 4→2, station max 4→2. **Tests**: new `tests/test_cpsat_post_passes.py` with 20+ assertions covering correctness (returns OPTIMAL on small fixtures), invariant preservation (partner pairs, opponent pairs, play counts, surrogate counts, cooldown violations unchanged after R/B flip and station permutation), budget validation (oversized clamped, zero/negative raises), empty-schedule handling, CP-SAT-never-loses-to-SA regression. **Live generation**: NOT yet wired into `generate_matches` — that needs async job handling (CP-SAT can take minutes; HTTP timeouts in proxy/route/browser need consideration) and UI budget picker, deferred to a follow-up session. Production users get CP-SAT-quality schedules via canonical library cache hits. **Doc**: `docs/scheduler/exact-post-passes.md` with budget guidance, model details, and empirical sweep results. 20 test suites pass. |
| **Live CP-SAT integration in Generate API + UI** (this session) | ✓ | The deferred follow-up from the previous session — wire CP-SAT post-pass into live `generate_matches`, expose via API + UI with budget picker. **Scheduler core**: `generate_matches()` and `_assign_unified()` gain `cpsat_post_pass_budget_s: float = 0.0` parameter. When positive, CP-SAT polish runs after the SA post-passes; budget split 10% R/B + 90% station. `run_iterations_worker()` accepts an 8-tuple form (n, mpt, cd, n_iter, worker_id, seed, weights, cpsat_budget) backwards-compatible with the existing 7-tuple and 6-tuple forms. CP-SAT polish runs ONCE per worker on the winner (not per iteration) — running it inside the loop would waste compute on schedules we throw away. **API surface**: `AbstractGenerateRequest` and `UnifiedScheduleRequest` get `cpsat_post_pass_budget_s: float = Field(0.0, ge=0.0, le=3600.0)`. Both call sites in `app/main.py` thread the budget into the worker tuple. `creation_provenance` JSONB column captures the budget so each schedule's audit trail records what polish was applied. **SSE phase event**: `POST /api/generate-abstract` emits a `{type:'phase', phase:'sa+cpsat_polish', cpsat_budget_s:N}` event upfront so the UI knows to display longer-wait copy. **OpenShift route timeout**: `openshift/05-route.yaml` bumped from 120s → 4000s. Without this, haproxy kills the SSE stream mid-solve for any budget over 2 min — keep-alive pings don't help because haproxy's timeout is wall-clock not idle. **UI**: Generate panel gains a "Quality polish" picker with 5 presets (Off / Quick 60s / Thorough 5min / Deep 15min / Maximum 60min) plus Custom (1-3600s). Each option's expected wall time is in its label. Hint text below updates as the user selects, surfacing what each choice does. `onCpsatBudgetChange()` handles toggle + hint update; `getCpsatBudgetForRequest()` resolves the picker to a clamped seconds value used in the POST body. Progress label updates to "Generating schedule, then CP-SAT polish (up to N min)…" when CP-SAT phase fires. **Tests**: `tests/test_cpsat_post_passes.py` extended with integration coverage — `generate_matches` budget=0 matches old behavior (regression test for the default path), `generate_matches` budget=5 preserves invariants, `run_iterations_worker` accepts all three tuple forms (6/7/8), Pydantic field validation (rejects > 3600, rejects negative). 20 test suites pass. **Deploy notes**: needs `--build` (backend changes), plus the route change requires reapplying `openshift/05-route.yaml` (the apply.sh script handles this). No DB migration. |
| **Construction-malformation fix for odd-team-count fixtures** (this session) | ✓ | The roadmap item framed as "three odd-team-count fixtures still error on the reference scheduler (2023mnmi 61t, 2024mndu 55t, 2025mnmi 51t)." Investigation revealed **two distinct bugs**, not one. **Bug A (reference scheduler adapter)**: cannot reproduce in dev container (binary not available). Current adapter omits `-u` flag as the 2026-05-09 FIX claimed, but the 2026-05-10 re-run reported persistent errors. To diagnose without re-running, beefed up `scripts/scheduler_eval/adapters/matchmaker.py` to capture BOTH stdout and stderr (last 500 bytes each) on non-zero exit — some MatchMaker versions write the error to stdout rather than stderr, leaving the old "stderr only" capture empty. Next eval run with the binary installed will reveal the actual exit-255 cause. **Bug B (our scheduler's construction phase)**: smoke-testing revealed 20-37% malformation rates on 51/55/61-team fixtures at MPT=7-8 — real MN regional and state-event sizes that production users hit. The old `ConstructionMalformedError` docstring claimed "≥16 teams 0%" which was **wrong**. Root cause: the FIRST surrogate model pre-picks surrogate teams with target=MPT+1, but `team_score` used raw `mc[t]` without normalizing for target — surrogate teams burned through their quota faster than regular teams (proportionally), exiting `under_quota` before the last match, leaving the construction with fewer than 6 teams to fill the final match. The legacy non-FIRST path had an at-quota fallback for exactly this case; the FIRST path didn't. **Fix in `app/scheduler.py`**: (1) `team_score` normalizes `mc[t]` by `target_count[t]` so surrogate and regular teams advance at the same proportional rate. Fast path when `target == matches_per_team` keeps bit-identical legacy behavior (FP division then multiplication can drift, changing sort tiebreaks and thus schedule-by-seed reproducibility). (2) FIRST phase 2 falls back to at-quota teams when `under_quota < 6`. Strictly preserves FIRST §10.5.2 placement on the common case; emergency over-quota appearance is strictly better than malformed match. **Post-fix rates** (30 seeds per fixture, verified): 51×7 20%→0%, 55×7 23%→0%, 55×8 37%→0%, 61×7 30%→0%, 61×8 20%→0%, 36×7 0%→0% (unchanged), 16×6 ~11% (pre-existing, unaffected by fix — the docstring's "≥16 teams 0%" claim was always wrong; tiny-fixture malformation is the fundamental tightness issue captured in Best Possible Schedule Phase 1 Q4). **Tests**: new `tests/test_construction_reliability.py` with 18 assertions covering 51/55/61×7/8 + 36×7 reliability + surrogate model correctness (total appearances = matches×6, all teams have mpt or mpt+1 appearances). 20 prior test suites still green; 21st suite is the new one. **Docs**: new `docs/scheduler/construction-malformation.md` with full investigation; `docs/scheduler/EVAL_FINDINGS.md` corrected from `[FIXED 2026-05-09]` to `[PARTIAL FIX 2026-05-09; STILL ERRORING 2026-05-10]` with context about the diagnostic upgrade. Updated `ConstructionMalformedError` docstring with corrected empirical data and acknowledgment that the prior measurement was incomplete. **Deploy notes**: needs `--build` (scheduler.py changes). No DB migration. No infrastructure changes. |
| **Bug A resolution: Fixture.total_matches off-by-one** (this session) | ✓ | Stark re-run with the beefed-up matchmaker adapter (previous session) finally surfaced the actual diagnosis for the persistent "MatchMaker exited 255" on 2023mnmi/2024mndu/2025mnmi. Three findings: (1) MatchMaker **exits cleanly** on all three fixtures with valid, full schedules. (2) The error was NOT in MatchMaker; it was in OUR adapter's parser. The parser validates `len(matches) == fixture.total_matches`, raising `ValueError("MatchMaker output has N matches; fixture expects N-1")` when they differ. (3) The mismatch was due to a 1-character bug in `scripts/scheduler_eval/harness_types.py:Fixture.total_matches`. **The bug**: `total_matches` used floor division (`a // b`) instead of ceil. For fixtures where `N×MPT` doesn't divide evenly into `2×TPA` (i.e., surrogate-required), floor returns one fewer than the schedule actually contains — the extra `N×MPT mod 6` slots get filled by surrogate teams in an additional final round. Examples: 51×9 = 459 slots / 6 = 76.5 → floor gives 76, actual 77. The docstring even said "if it doesn't come out to an integer, a surrogate round can balance it" — but the code didn't actually do the round-up. **Fix**: change to `(total_slots + slots_per_match - 1) // slots_per_match` (integer ceil idiom). **Diagnostic chain explained**: the "exit 255" framing came from an early eval report that surfaced only the exception type/message, then downstream renderers compressed it to "ERROR" without context. The 2026-05-09 `-u` fix was correct but addressed a different (earlier) failure mode; once that fix landed, MatchMaker started producing output that then hit the total_matches mismatch in our adapter. **Tests**: new `tests/test_fixture_total_matches.py` with 24 assertions covering even-division shapes (12×6, 36×7, 24×8, 60×12 etc.) and surrogate-required shapes (51×9, 55×9, 61×9 plus MPT=7/8 variants). Includes structural invariant `total_matches × 6 ≥ num_teams × matches_per_team` so the bug can't regress in the other direction either. 22 test suites pass. **Doc**: `docs/scheduler/EVAL_FINDINGS.md` rewritten — `[PARTIAL FIX 2026-05-09; STILL ERRORING 2026-05-10]` → `[RESOLVED 2026-05-14]` with full diagnostic chain. **Deploy notes**: change is in `scripts/scheduler_eval/` only (eval harness, not production). No `--build` needed for production app; the eval re-run on Stark can be done after `git pull` with no rebuild. No DB migration. |
| **Eval composite gap: Stark validation + weight sensitivity** (this session) | ✓ | Follow-up to the eval composite gap adapter fixes. Validated on Stark with full eval harness + matchmaker binary; resolved environment quirks; ruled out weight tuning as a path to closing the residual gap. **Environment quirks resolved**: (1) ortools missing from venv → installed via `pip install ortools` in the activated venv. (2) Python 3.14 → pydantic-core 2.27.1 has no 3.14 wheel; rebuilt venv on Python 3.13.13 where wheels exist. Recommended for Stark dev going forward (3.14 needs `pydantic>=2.12` which is a production-touching upgrade we declined for this session). (3) Multiprocessing diagnostic spam from heredoc spawn-mode — documented as test-only quirk; production runner uses fork. **Eval results on 2023mnmi (61×9) post-fix**: min_match_gap 3→**7** ✓, max_color_imbalance 3→**1** ✓, max_station_spread 5→2 (improved but harness's 3-pos metric still flags as poor; our framework's 6-pos metric hits 1). But **repeat_opponents regressed 16→86** as predicted by Q1, and composite barely moved (40.30→40.10, net 1 point). MM stays at 30.30. The clean wins on three metrics get eaten by the regression on opponent diversity. **Weight sensitivity sweep (61×9, ideal_gap=7, 500K SA, seed=42)**: W_OPPONENT varied 60→100→200→400 (default → 6.7×). repeat_opp stayed in 88-93 range (noise band ±3); max_opp_rep got *worse* at 400 (3→4, classic overweighting signature). **Conclusion**: SA is structurally gap-constrained on these fixture shapes — not under-weighting opponents. The legal-swap space at ideal_gap=7 is too small for SA to find both gap=7 AND opponent_diversity≈MM's 15. Parameter tuning ruled out as a lever. **What's left for the gap**: real algorithmic work (CP-SAT single-stage construction matching MM's approach, OR a CP-SAT opponent-diversity post-pass). Captured as Best Possible Schedule workstream Phase 1 Q4. **Doc updates**: `docs/scheduler/eval-composite-gap.md` extended with weight-sensitivity table and updated Q1 framing (rules out weight tuning, names the remaining options). **Deploy notes**: doc-only changes this round; no code changes. No `--build`. No DB migration. **Status**: ✓ closes the investigation. The actual gap-closure work belongs in BPS. |
| **Personal Access Tokens + CLI client** (this session) | ✓ | Shipped server-side PAT auth and an independently-versioned CLI/SDK package. **Server side (in this repo)**: (1) New `app/personal_access_tokens.py` module — `mint_pat`, `verify_pat`, `revoke_pat`, `list_pats`; SHA-256 hashed (not bcrypt — PATs are 256-bit cryptorandom, bcrypt would add 100ms/req); `PAT_PREFIX = "frc_pat_"`; tokens are `frc_pat_<43chars base64url>` for 256 bits of entropy. (2) New `PersonalAccessToken` model in `app/db.py` — FK to users, `token_prefix` (12 chars displayed in UI), `token_hash` (unique-indexed), `scopes` (JSON, default `["*"]`, no enforcement yet), `is_admin` snapshotted at mint, optional `expires_at`/`revoked_at`/`last_used_at`. Indexes on token_hash (hot path) + user_id + partial (user_id, created_at) WHERE revoked_at IS NULL. (3) `app/auth.py:get_current_user` extended — now takes a `db` session via Depends, dispatches by prefix (`frc_pat_` → PAT verify, else JWT decode). Returns shape-equivalent claims dict regardless of path. (4) Three new endpoints in `app/main.py`: `POST /api/me/tokens` (mint, returns plaintext ONCE), `GET /api/me/tokens?include_revoked=` (list, no plaintext), `DELETE /api/me/tokens/{id}` (revoke, idempotent, 204). All three require **JWT** auth via new `_require_jwt_auth` dependency — PATs cannot manage PATs (defense against forest growth from leaked tokens). (5) Migration `scripts/migrations/20260515_add_personal_access_tokens.sql`. (6) New `tests/test_personal_access_tokens.py` — 38 assertions with MockSession introspecting SQLAlchemy where clauses (no Postgres dependency). All 23 server test suites green (18 Python + 4 JS + smoke). **Client side (new sibling repo `frc-scheduler-client/`)**: independently versioned at 0.1.0, pip-installable, depends only on `click` `httpx` `rich`. Layout: `src/frc_scheduler_client/{__init__.py, client.py, auth.py, cli.py, _helpers.py, exceptions.py}` plus `commands/{auth.py, events.py, schedules.py, tokens.py}`. **SDK** (`FrcSchedulerClient`): synchronous wrapper around httpx.Client; methods grouped by resource (`auth_*`, `tokens_*`, `events_*`, `schedules_*`); raises typed exceptions (`AuthenticationError`, `NotFoundError`, `APIError`) on non-2xx; 5-minute default read timeout (covers CP-SAT generation). **CLI** (`frc` command via setuptools entry point): commands include `frc login`, `frc logout`, `frc whoami`, `frc ping`, `frc config-show`, `frc tokens {list,create,revoke}`, `frc events {list,show,create}`, `frc schedules {list,show,generate,quality}` with `--json` and `--save-to` flags where useful. Credentials in `~/.config/frc-scheduler/credentials.json` mode 0600 with atomic write; env-var overrides (`FRC_SERVER`/`FRC_TOKEN`); exit codes 0/1/2/3/4/130 mapped to client exception types. **Client tests**: 46 (`test_auth.py` for credential storage with chmod check, `test_client.py` for HTTP layer via `httpx.MockTransport`, `test_cli.py` for command parsing via `click.testing.CliRunner` with patched `_helpers.FrcSchedulerClient`). All 46 client tests green. **Roadmap**: v0.1 ships PAT auth + core commands; v0.2 adds OAuth device flow login (so users can log in without first minting a PAT in the web UI); v0.3 fixture/quality/admin commands; v0.4 PDF import + bulk ops. **Deploy**: this round needs **--build** (server code changes) AND **DB migration** (`psql $DATABASE_URL -f scripts/migrations/20260515_add_personal_access_tokens.sql`) applied BEFORE the pod restarts. Client package is a separate distribution; install with `pip install -e .` from the `frc-scheduler-client/` checkout. **Open follow-ups**: (a) web UI to manage PATs (`/static/index.html` user menu → API tokens modal) — server endpoints work but no UI to call them yet, so users must use curl or the CLI to mint their first PAT. (b) OAuth device flow on the server side (RFC 8628: device_code endpoint, verify page, token endpoint with 5s polling). (c) Once device flow ships, `frc login` (no `--token`) becomes the canonical login. |
